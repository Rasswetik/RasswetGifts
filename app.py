...
_portal_connection_state = {'connected': False, 'last_check': 0.0, 'last_error': None, 'last_latency_ms': None}
_portal_sync_thread = None
_portal_sync_thread_lock = threading.Lock()


def _get_portal_auth(force_reload=False):
    """Единый источник authData для всего Portal-кода."""
    global _portal_auth_data
    with _portal_auth_lock:
        if _portal_auth_data and not force_reload:
        ...
        if token.startswith('tma '):
            token = token[4:].strip()
        ok, _err = _portal_validate_token(token)
        if not ok:
            _portal_auth_data = None
            return None
...
def _set_portal_connection(connected, error=None, latency_ms=None):
    _portal_connection_state.update({
...
def _portal_save_token(token):
    """Сохраняет токен в файл + env и обновляет единое состояние авторизации."""
...
def _portal_delete_token():
    ...
    _portal_auth_data = None
    _set_portal_connection(False, 'Токен удалён')
...
def start_portal_price_sync_loop():
    """Запускает единственный фоновой sync Portal."""
    global _portal_sync_thread
    if not PORTAL_SYNC_ENABLED:
        logger.info('Portal auto-sync disabled via PORTAL_SYNC_ENABLED')
        return None
    with _portal_sync_thread_lock:
        if _portal_sync_thread and _portal_sync_thread.is_alive():
            return _portal_sync_thread

        def sync_loop():
            logger.info(f'🔄 Portal sync thread started ({PORTAL_SYNC_INTERVAL_MINUTES} min)')
            while not _db_ready:
                time.sleep(1)
            # Быстрая проверка соединения, затем первая синхронизация.
            time.sleep(10)
            try:
                ok, err = _portal_check_connection()
                if ok:
                    _portal_sync_floors()
                else:
                    logger.warning(f'⚠️ Portal initial check failed: {err}')
            except Exception as e:
                logger.warning(f'Portal initial sync error: {e}')

            while True:
                try:
                    interval = max(1, int(PORTAL_SYNC_INTERVAL_MINUTES)) * 60
                    # Проверяем соединение чаще, чем обновляем каталог.
                    ...
                    if ok:
                        ...
                        if not result.get('success'):
                            logger.warning(f'⚠️ Portal sync failed: {result.get("error")}')
                    else:
                        logger.warning(f'⚠️ Portal connection unavailable: {err}')
                        # Короткий retry, чтобы восстановиться без перезапуска приложения.
                        for _ in range(3):
                            time.sleep(20)
                            ok, err = _portal_check_connection()
                            if ok:
        ...
        _portal_sync_thread = threading.Thread(target=sync_loop, name='portal-sync-loop', daemon=True)
        _portal_sync_thread.start()
        return _portal_sync_thread
...
# ─── Portal Marketplace Integration (единый клиент через aportalsmp) ─────
# Все Portal-операции используют один authData и один async runner.
# Прямой HTTP-клиент здесь намеренно не используется: это убирает рассинхрон
# между /collections, /search, sync и buy/transfer.
PORTAL_API_BASE = 'https://portal-market.com/api/v1'


def _portal_call(fn, **kwargs):
    """Безопасный sync-wrapper для async aportalsmp-функций."""
    if not _APORTALSMP_AVAILABLE or fn is None:
        return None, 'Модуль aportalsmp не установлен'
    try:
        loop = _portal_asyncio.new_event_loop()
        try:
            _portal_asyncio.set_event_loop(loop)
            result = loop.run_until_complete(
                _portal_asyncio.wait_for(fn(**kwargs), timeout=60)
            )
            return result, None
        finally:
            try:
            ...
            loop.close()
            try:
                _portal_asyncio.set_event_loop(None)
            except Exception:
                pass
    except _portal_asyncio.TimeoutError:
        return None, 'Portal timeout: сервер не ответил за 60 сек'
    except Exception as e:
        return None, str(e)


def _portal_check_connection():
    """Реальная проверка Portal, а не просто наличие токена."""
    token = _get_portal_auth(force_reload=True)
    if not token:
        _set_portal_connection(False, 'Токен Portal не задан')
        return False, 'Токен Portal не задан'
    if not _APORTALSMP_AVAILABLE:
        _set_portal_connection(False, 'Модуль aportalsmp не установлен')
        return False, 'Модуль aportalsmp не установлен'

    started = time.perf_counter()
    result, err = _portal_call(_portal_collections_fn, authData=token, limit=1)
    latency = round((time.perf_counter() - started) * 1000, 1)
    if err:
        _set_portal_connection(False, err, latency)
        return False, err
    items = _portal_extract_items(result)
    if not items:
        err = 'Portal доступен, но не вернул коллекции'
        _set_portal_connection(False, err, latency)
        return False, err
    _set_portal_connection(True, None, latency)
    return True, None


def _portal_collections(limit=200, offset=0):
    """Загружает коллекции Portal через единый aportalsmp-клиент."""
    token = _get_portal_auth()
    if not token:
        return False, 'Portal auth не настроен'
    result, err = _portal_call(
        _portal_collections_fn,
        authData=token,
        limit=max(1, min(int(limit), 500)),
    )
    if err:
        # Один автоматический retry с принудительной перезагрузкой токена.
        token = _get_portal_auth(force_reload=True)
        if token:
            result, err = _portal_call(_portal_collections_fn, authData=token, limit=max(1, min(int(limit), 500)))
        if err:
            _set_portal_connection(False, err)
            return False, err
    ...
    for coll in items:
        try:
            ...
            if not short_name:
                continue
            result_rows.append({
                'id': coll.get('id', '') if isinstance(coll, dict) else getattr(coll, 'id', ''),
                'name': name or short_name,
                'short_name': short_name,
                'floor_price': float(floor_price or 0),
                'photo_url': photo_url,
                'supply': (coll.get('supply', 0) if isinstance(coll, dict) else getattr(coll, 'supply', 0)) or 0,
                'listed_count': (coll.get('listed_count', 0) if isinstance(coll, dict) else getattr(coll, 'listed_count', 0)) or 0,
                'day_volume': float((coll.get('day_volume', 0) if isinstance(coll, dict) else getattr(coll, 'day_volume', 0)) or 0),
                'volume': float((coll.get('volume', 0) if isinstance(coll, dict) else getattr(coll, 'volume', 0)) or 0),
                'market_cap': float((coll.get('market_cap', 0) if isinstance(coll, dict) else getattr(coll, 'market_cap', 0)) or 0),
                'is_premarket': bool(coll.get('is_premarket') if isinstance(coll, dict) else getattr(coll, 'is_premarket', False)),
                'is_new': bool(coll.get('is_new') if isinstance(coll, dict) else getattr(coll, 'is_new', False)),
            })
        except Exception as e:
            logger.debug(f'Portal collection parse error: {e}')
    if result_rows:
        _set_portal_connection(True, None)
    return True, result_rows


def _portal_search(collection_short_name, limit=20, model=None, backdrop=None, symbol=None):
    """Ищет NFT через aportalsmp, с повтором после обновления authData."""
    token = _get_portal_auth()
    if not token:
        return False, 'Portal auth не настроен'
    kwargs = {
    ...
    if collection_short_name:
        kwargs['gift_name'] = collection_short_name
    if model:
        kwargs['model'] = model
    if backdrop:
    ...
    result, err = _portal_call(_portal_search_fn, **kwargs)
    if err:
        token = _get_portal_auth(force_reload=True)
        if token:
            kwargs['authData'] = token
            result, err = _portal_call(_portal_search_fn, **kwargs)
    if err:
        _set_portal_connection(False, err)
        return False, err
    ...
    rows = []
    for gift in (result or []):
        try:
            gid, number, price, gift_model, gift_backdrop, gift_symbol = _portal_extract_gift_fields(gift)
            coll_name = getattr(gift, 'name', '') or (gift.get('name', '') if isinstance(gift, dict) else '') or collection_short_name
            rows.append({
                'id': gid,
                'number': number,
                'price_ton': float(price or 0),
                'model': gift_model,
                'backdrop': gift_backdrop,
                'symbol': gift_symbol,
                'image': _portal_nft_image_url(collection_short_name, number) if number else (getattr(gift, 'photo_url', '') if not isinstance(gift, dict) else gift.get('photo_url', '')),
                'collection_name': coll_name,
                'title': (getattr(gift, 'title', '') if not isinstance(gift, dict) else gift.get('title', '')) or (f'{coll_name} #{number}' if number else coll_name),
            })
        except Exception as e:
...
def _portal_sync_floors():
    """Синхронизация floor-цен Portal → gifts.json с сохранением последнего успешного кэша."""
    ...
    for c in colls:
        ...
        item = {
            ...
            'image': c.get('photo_url') or _portal_collection_image_url(slug),
            'source': 'portal',
        }
        floor = float(c.get('floor_price') or 0)
        if floor > 0:
            item['fragment_price_ton'] = round(floor, 4)
            item['value'] = int(round(floor * 100))
        portal_catalog.append(item)

    if not portal_catalog:
        return {'success': False, 'error': 'Portal не вернул пригодных коллекций'}
    ...
    try:
        ...
        updated = 0
        for item in portal_catalog:
            target = by_slug.get(item['fragment_slug'])
            if target is None:
                gifts.append({**item, 'id': None})
                updated += 1
                continue
            changed = False
            for key in ('fragment_slug', 'fragment_url', 'image', 'fragment_price_ton', 'value'):
                if key in item and target.get(key) != item[key]:
                    target[key] = item[key]
                    changed = True
            if changed:
                updated += 1

        tmp_path = gifts_path + '.tmp'
        with open(tmp_path, 'w', encoding='utf-8') as f:
            json.dump({'gifts': gifts} if wrap_dict else gifts, f, ensure_ascii=False, indent=2)
        os.replace(tmp_path, gifts_path)
        gifts_cache = None
        gifts_cache_time = None
    except Exception as e:
        return {'success': False, 'error': f'Ошибка сохранения gifts.json: {e}'}
    ...
    try:
        ...
        pass
    logger.info(f'✅ Portal sync: обновлено {updated}/{len(gifts)}, коллекций {len(portal_catalog)}')
...
def portal_status_legacy():
    """Статус подключения к Portal с реальным ping."""
    try:
        _portal_check_connection()
        token = ''
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
...
def portal_connect():
    ...
    try:
        ...
        auth = _get_portal_auth(force_reload=True)
        if not auth:
            return jsonify({'success': False, 'error': 'Portal token отсутствует или имеет неверный формат'})
        ok, err = _portal_check_connection()
        if not ok:
            return jsonify({'success': False, 'error': err, 'connected': False})
        return jsonify({'success': True, 'message': 'Portal подключён и отвечает', 'connected': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def portal_info():
    ...
    try:
        auth = _get_portal_auth()
        connected = bool(_portal_connection_state.get('connected'))
        if auth and (time.time() - _portal_connection_state.get('last_check', 0) > 30):
            connected, _ = _portal_check_connection()
...
def portal_search_gifts():
    """Search Portal marketplace using the same authenticated client as all other routes."""
    name = _portal_clean_name(request.args.get('name', ''))
    if not name:
        return jsonify({'success': False, 'error': 'name required'})
    try:
        ok, gifts = _portal_search(name, limit=20)
        if not ok:
            return jsonify({'success': False, 'error': gifts})
        floor_price = min((g['price_ton'] for g in gifts if g['price_ton'] > 0), default=0)
        return jsonify({
            'success': True,
            'gifts': gifts,
            'floor_price_ton': round(floor_price, 4),
        })
    except Exception as e:
